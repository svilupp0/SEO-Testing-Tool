...
describe('Test 1.1 & 1.2 - OAuth2 Flow Completo (REALE)', () => {
  ...
  it.skipIf(!GOOGLE_AUTH_CODE)('STEP 2: Dovrebbe scambiare CODE con TOKEN', async () => {
    const config = getTestCredentials();
  ...
  it.skipIf(!GOOGLE_ACCESS_TOKEN)('STEP 3: Dovrebbe recuperare info utente', async () => {
    const config = getTestCredentials();
    ...
    try {
      ...
      console.log('   Email Verificata: ' + (userInfo.verified_email ? 'Si' : 'No'));
  ...
  it.skipIf(!GOOGLE_REFRESH_TOKEN)('STEP 4: Dovrebbe rinnovare token con refresh token (TEST 1.2)', async () => {
    const config = getTestCredentials();
    ...
    try {
      const newTokens = await tokenManager.refreshAccessToken(GOOGLE_REFRESH_TOKEN);
