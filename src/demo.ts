...
function generateMetrics(
  ...
  const rows: { testId: string; date: string; clicks: number; impressions: number }[] = [];
  for (let i = 0; i < days; i++) {
    const clicks = Math.max(0, Math.round(rng.normal(clickMean, clickStdDev)));
    const impressions = Math.max(
      clicks,
      Math.round(rng.normal(clicks * impressionMultiplier, clicks * impressionMultiplier * 0.15)),
    );
    rows.push({
      testId,
